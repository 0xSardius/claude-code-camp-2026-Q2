"""Port of week1_baseline/ruby/12_context/lib/boukensha/backends/ollama.rb --
adds "think": False to the payload and reasoning-block normalization to
parse_response. MODELS table shrinks to just "gemma4:e4b" -- matches
Ruby's current source exactly (every other local-model entry 10_standard_tool_library/
11_tui had is gone); ported as the literal current table, not merged
...
class Ollama(Base):
    ...
    def __init__(self, *, host="http://localhost:11434", model):
        super().__init__()
        self._host = host
    ...
    # (the inverse of parse_response). Text-only turns are stored as a
    # bare string, so wrap it back into a single text block before
    # filtering.
