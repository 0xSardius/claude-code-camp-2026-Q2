"""Port of week1_baseline/ruby/10_standard_tool_library/lib/boukensha/tools/file_system.rb --
...
def register(registry, *, working_dir):
    ...
    def list_directory(path="."):
        ...
        # Ruby: Dir.entries(target).reject{".."}.sort.map{...} -- Python's
        # Path.iterdir() never yields "."/".." at all, so the reject step
        # has nothing to do here, not a gap.
    ...
    def search_files(pattern, path=".", glob="*"):
        ...
        for f in files:
            ...
            try:
                ...
                # Ruby: `rescue => e` -- broad, so one unreadable/undecodable
                # file (e.g. UnicodeDecodeError on a binary file) appends an
                # inline error line and the loop continues, rather than
                # discarding every match already found in this call.
