"""Port of week1_baseline/ruby/05_agent_loop/lib/boukensha/backends/openai.rb --
adds tools= override on to_payload, parse_response, and private
_assistant_message. OpenAI is the one backend whose tool-call arguments
arrive as a JSON-encoded string, not a structured object -- needs
json.loads/json.dumps where the others don't (see
...
class OpenAI(Base):
    ...
    def parse_response(self, response):
        ...
        content = []
        # Ruby: `if message["content"]` -- nil-only falsy, so an empty
        # string still passes. `is not None`, not a truthy check, or an
        # empty-string content block silently vanishes vs. Ruby's version.
