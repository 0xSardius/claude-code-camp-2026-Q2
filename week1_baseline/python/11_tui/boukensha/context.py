"""Port of week1_baseline/ruby/10_standard_tool_library/lib/boukensha/context.rb --
adds working_dir (see docs/plans/python_port/10_standard_tool_library).
...
class Context:
    def __init__(self, *, task, system: str | None = None, working_dir: str | bool | None = None) -> None:
        ...
        # DELIBERATE truthy check, not a nil-check. Boukensha.run/.repl pass
        # working_dir: false as the explicit opt-out sentinel, and `false`
        # needs to map to "no working dir" here. A bare Python `if
        # working_dir:` is actually CORRECT in this one spot (False is
        # falsy in Python too) -- unlike every other ||/truthy-check in
        # this codebase, this is not a bug to fix with `is not None`.
