"""Port of week1_baseline/ruby/05_agent_loop/lib/boukensha/tasks/base.rb --
byte-identical to 04_api_client's version except the addition of
max_iterations/max_output_tokens and the private integer_setting helper
backing them (see docs/plans/python_port/05_agent_loop). Keep this a
literal mirror of the Ruby Tasks::Base class; re-read the Ruby source
...
class Base:
    ...
    @classmethod
    def _integer_setting(cls, settings: dict, key: Any, default: int) -> int:
        # Ruby's Integer(value) raises ArgumentError/TypeError on a
        # non-integer-parseable value; Python's int(value) raises
        # ValueError/TypeError similarly. Neither language's example
