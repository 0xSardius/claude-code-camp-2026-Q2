"""Port of week1_baseline/ruby/08_the_repl_loop/lib/boukensha/agent.rb --
now persists every final reply into context on all three exit paths (run's
success path, _wrap_up's success path, and its ApiError fallback) -- a
REPL needs the full transcript so later turns see prior exchanges, unlike
a one-shot run where the context was thrown away anyway. See
docs/plans/python_port/08_the_repl_loop.

interrupt_event (new in docs/plans/python_port/11_tui) has NO Ruby
counterpart -- it's the Python-side answer to Ruby's
Thread#raise(Interrupt), which has no safe Python equivalent (no
supported API to kill an arbitrary running thread). Checked once per
iteration, at the top of run()'s loop, so a running turn stops at the
next iteration boundary rather than being interruptible mid-API-call the
way Ruby's thread-level interrupt theoretically can. Only ever passed by
Tui; the plain non-TUI REPL and boukensha.run() never pass it, so this is
a strict addition with zero behavior change for every other caller.
"""
...
class Agent:
    ...
    # path), which falls back to this constant. 0 disables the ceiling.
    MAX_ITERATIONS = 25
    ...
    def __init__(
        ...
        *,
        ...
        # once, at def-time. logger=None + lazy-construct here is the only
        # way to get Ruby's actual per-call-fresh-instance semantics --
        # see the port plan's dedicated section on this gotcha. A naive
        # `logger=Logger()` in the signature would construct ONE Logger at
        # import time and silently share it across every Agent that
    ...
    def run(self):
        while True:
            # Cooperative-cancellation checkpoint (see module docstring) --
            # checked before starting a new iteration, not mid-call.
    ...
    def _call_opts(self):
        # Ruby: @max_output_tokens ? {...} : {} -- a truthy check, so an
        # explicit max_output_tokens: 0 is still included (0 is truthy in
        # Ruby). `is not None`, not a bare truthy check, for the same
        # reason as every backend's tools:-override handling.
    ...
    def _handle_tool_calls(self, content, response):
        ...
        for block in tool_calls:
            ...
            # try/except/else, not a single try/except -- the success-path
            # logger call must NOT be covered by the except, or a logging
            # failure (e.g. disk I/O writing the session log) after a
            # genuinely successful dispatch gets misreported to the model
            # as a tool failure, discarding the real result. Same bug
            # existed in the Ruby source (both the dispatch call and its
            # success-path logger.tool_result were inside one begin/rescue)
            # -- fixed there too, not just mirrored here.
            try:
