"""Port of week1_baseline/ruby/11_tui/lib/boukensha.rb -- adds tui=True to
repl() (see docs/plans/python_port/11_tui).
...
logic between self.run/self.repl either (confirmed near-total duplication
between the two Ruby methods), and this project mirrors Ruby's own
duplication rather than deduplicate on its behalf -- same precedent as
...
Ruby's `Boukensha.quiet!`/`.loud!`/`.quiet?` were removed entirely at the
module level this step (confirmed via diff -- a real deletion, not a
regression). `_module_state.py`'s `enable_quiet`/`disable_quiet`/
`is_quiet` are left in place and still exported below: they were already
established as permanently unused/no-op since `08`, and removing a
function nothing calls is a separate, unrelated cleanup this port doesn't
need to make just because Ruby happened to make it this step.
"""
import os
import threading
...
def run(
    *,
    ...
    # Ruby's `mud || default` only falls back on nil (false is already
    # excluded above) -- an explicit `mud={}` is truthy in Ruby (kept
    # as-is) but falsy in Python, so a bare `mud or default` would
    # silently discard it and fall back to config instead. `is not None`,
    # not `or`, for the same reason as every other Ruby-truthy-vs-Python-
    # falsy translation in this codebase.
