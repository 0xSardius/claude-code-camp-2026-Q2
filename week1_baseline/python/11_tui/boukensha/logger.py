"""Port of week1_baseline/ruby/07_the_run_dsl/lib/boukensha/logger.rb.
...
turn()/subscribe() are new this step -- declared in the Ruby source but
not called anywhere in it (same "declared but currently unused" pattern
as LoopError/quiet!/loud! in earlier steps), ported for completeness.
_write_log passes subscribers the ORIGINAL event dict, without the
session_id/at fields merged in only for the JSONL line written to disk --
matches Ruby exactly, not "fixed" (see docs/plans/python_port/07_the_run_dsl).
"""
import json
import re
import secrets
...
class Logger:
    ...
    def __init__(self, *, session_id=None, dir=None, log=None, snapshot=None):
        # Ruby: @subscribers ||= [] lazily inits on first subscribe() call.
        # Python just initializes it here -- __init__ is already the
        # natural single init point, no behavior difference (an empty
        # list iterated in _write_log is already a no-op either way).
        self._subscribers = []
    ...
    def _write_log(self, event):
        ...
        # event, not `record` (session_id/at are never seen by subscribers,
        # only by the JSONL line on disk). Not "fixed" -- ported exactly.
