    ...
    module Shell
      def self.register(registry, working_dir:, timeout: 30, allowed_commands: nil)
        ...
        registry.tool "run_command",
          ...
          # command runs via a shell (Open3.capture2e with a single string
          # argument), so checking only the first token isn't enough on its
          # own -- "git status; rm -rf ~" would pass an allow-list containing
          # "git" and then the shell would execute the rest anyway. Reject
          # shell metacharacters too so the allow-list can't be chained past.
          if allowed_commands
            if command.to_s =~ /[;&|><`$\n]|\$\(/
