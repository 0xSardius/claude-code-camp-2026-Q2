"""Port of week1_baseline/ruby/12_context/lib/boukensha/backends/openai.rb --
a full rewrite, not an incremental port: gpt-5.x rejects
`reasoning_effort` + tools on /v1/chat/completions ("Please use
/v1/responses"), so this backend now targets the Responses API instead of
...
`{role: "tool"}` message. MODELS table drops gpt-5.4 (present in 11_tui,
absent from Ruby's 12_context source) -- matches Ruby exactly, not merged
forward.
...
class OpenAI(Base):
    ...
    def parse_response(self, response):
        ...
        for item in response.get("output") or []:
            ...
            if item_type == "reasoning":
                ...
                text = "".join(
                    c.get("text") or "" for c in (item.get("content") or []) if c.get("type") == "output_text"
                )
                if text != "":
    ...
    def _assistant_items(self, content):
        ...
        items = [] if text == "" else [{"role": "assistant", "content": text}]
